...
class ValidEmail extends StatefulWidget {
  ...
  _ValidEmailState createState() => _ValidEmailState();
}

class _ValidEmailState extends State<ValidEmail> {
  final TextEditingController _emailController = TextEditingController();
  bool login = false;
  bool isLoading = false;
  StateEnum? state;
  signIn(String email) async {
    String url = '${ApiEndPoint.email_verification}?email=$email';
    ...
    if (response.statusCode == 200) {
      if (jsonDecode(response.body)['code'] == 500) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => ValidOTP(value: _emailController.text)),
        );
      }
    } else {
      // ignore: use_build_context_synchronously
                ...
                child: TextFormField(
                  ...
                  validator: (value) {
                    bool passValid = RegExp(
                        ...
                        .hasMatch(value!);
                    if (value.isEmpty) {
                      return 'Enter your New Email Id';
                    } else if (!passValid) {
                      return 'Enter your New Email Id';
                    }
                    return null;
