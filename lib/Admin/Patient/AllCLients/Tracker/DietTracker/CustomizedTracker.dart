...
class DietCustomized extends StatefulWidget {
  String? customerid;
  DietCustomized(this.customerid);
  @override
  DietCustomizedState createState() => DietCustomizedState();
}

class DietCustomizedState extends State<DietCustomized> {
  TextEditingController _urlController = TextEditingController();
  bool isLoading = true;
  postData(String url) async {
    int CustomerId = widget.customerid as int;
    String url = Sales.customized_diet;
    Map body = {
    ...
    if (token != null) {
      ...
      if (res.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Diet Plan Submitted Successfully'),
          backgroundColor: Colors.green,
        ));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Diet Plan Submitted ,Please try again'),
  ...
  Widget _buildNote() {
    return Padding(
      ...
      child: Text(
        'To Upload the Customized Diet, Please Paste the Google Drive Link Here',
      ...
      children: <Widget>[
        Container(
          ...
          child:ElevatedButton(
            ...
            onPressed:
            _urlController.text == "" + widget.customerid.toString() + "diet"
                ? null
                : () {
              setState(() {
                isLoading = true;
